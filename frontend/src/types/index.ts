...
export interface AuditEvent {id:number;event_type:string;message:string;event_metadata:Record<string,unknown>|null;created_at:string}
export interface InvoiceDetail extends Invoice {po_number:string|null;invoice_date:string|null;file_path:string;extraction_confidence:string|null;updated_at:string;purchase_order:PurchaseOrder|null;exceptions:InvoiceException[];audit_events:AuditEvent[]}
